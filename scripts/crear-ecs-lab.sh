...
#####################################
# 0. OBTENER RED (creada por crear-red-lab.sh)
#####################################
...
echo "VPC: $VPC_ID"
echo "Public: $SUBNET_PUB_A / $SUBNET_PUB_B"
echo "App: $SUBNET_APP_A / $SUBNET_APP_B"
  ...
  --container-definitions "[
    {
      ...
      \"environment\": [
        ...
        {\"name\": \"DB_HOST\", \"value\": \"tienda-db.tienda.local\"},
...
#####################################
# 5. SERVICE DISCOVERY (DNS interno para DB)
#####################################
echo "5. Service Discovery..."

NAMESPACE_ID=$(aws servicediscovery list-namespaces \
  --query "Namespaces[?Name=='tienda.local'].Id" --output text)
...
if [ -z "$NAMESPACE_ID" ]; then
  ...
  echo "Esperando creacion namespace..."
  sleep 30
  NAMESPACE_ID=$(aws servicediscovery list-namespaces \
    --query "Namespaces[?Name=='tienda.local'].Id" --output text)
fi
...
SD_DB=$(aws servicediscovery list-services \
  --query "Services[?Name=='tienda-db'].Arn" --output text)

if [ -z "$SD_DB" ]; then
  SD_DB=$(aws servicediscovery create-service \
    --name tienda-db \
...
if [ "$DB_EXISTS" != "ACTIVE" ]; then
  aws ecs create-service \
    ...
    --service-registries "registryArn=$SD_DB" \
    --region $REGION > /dev/null
fi
echo "Servicio DB ok."
